...
export function MobileTaskSheet({ onClose, onDelete, onDone, onDuplicate, onEdit, onMove, projects = [], task }) {
  ...
  return (
    <div className="absolute inset-0 z-40 overflow-y-auto bg-[#F5F5FA] px-5 py-6">
      ...
      <div className="mt-16">
        <p className="type-mobile-meta">{getProjectLabel(task, projects)} - {getPhaseLabel(task, projects)}</p>
        <h2 className="mt-4 text-3xl font-medium leading-tight tracking-normal">{task.title}</h2>
        <p className="type-mobile-body mt-5 text-base leading-7">{task.notes || task.detail || 'No detail added.'}</p>
    ...
    <div className="fixed inset-0 z-50 flex items-end bg-[rgba(33,33,33,0.25)]">
      <div className="w-full rounded-t-[32px] bg-white p-5">
        ...
        <label className="block">
          ...
          />
          <span className="type-mobile-body mt-2 block text-xs">Write naturally. Project and date are detected before saving.</span>
        </label>
        {parsedTask && (
          <div className="mt-4 rounded-[22px] bg-[#DBDFE9]/60 p-4 text-sm leading-6 text-[#777777]">
            <p className="type-mobile-meta mb-1 text-[#212121]">Detected</p>
