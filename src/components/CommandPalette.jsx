...
export function CommandPalette({ commands, isOpen, onClose, onOpen }) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-start justify-center bg-studio-ink/20 px-4 py-20 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Command palette">
      ...
      <div className="relative z-10 w-full max-w-2xl overflow-hidden rounded-lg border border-black/[0.08] bg-studio-bone/90 backdrop-blur-xl">
        <div className="flex min-h-16 items-center rhythm-control-gap border-b border-black/[0.06] px-5">
          ...
          />
          <span className="type-control rounded-sm border border-black/[0.08] px-2.5 py-1 text-studio-muted">
            {shortcutLabel}
          </span>
        </div>
        ...
        <div className="max-h-[420px] overflow-y-auto p-2" role="listbox" aria-label="Commands">
          {filteredCommands.map((command, index) => {
            ...
            return (
              <button
                key={command.id}
                className={`flex min-h-14 w-full items-center justify-between rhythm-stack-tight rounded-md px-4 py-3 text-left transition-all duration-150 ${
