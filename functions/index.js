...
function normalizeProject(input = {}) {
  const payload = input.payload && typeof input.payload === 'object' ? input.payload : input.project || input;
  const project = {
    ...payload,
  };
...
function getProjectName(command = {}) {
  return command.projectName || command.name || command.payload?.name || command.project?.name || '';
}
